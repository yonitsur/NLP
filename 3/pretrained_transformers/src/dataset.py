...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #select mask_content length
        
        mean = len(document) // 4
        mask_len = mean + random.randint(-mean, mean)
        i = random.randint(0, mask_len)
        masked_content = document[i:i+mask_len]
        prefix = document[:i]
        suffix = document[i+mask_len:]
        
        assert len(prefix) + len(suffix) + len(masked_content) == len(document)
        ...
        dixi = [self.stoi[s] for s in input_string]
        dixo = [self.stoi[s] for s in output_string]

        
        x = torch.tensor(dixi, dtype=torch.long)
        y = torch.tensor(dixo, dtype=torch.long)
        return x, y
